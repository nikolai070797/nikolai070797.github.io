...
import { MegaMenu4, MegaMenu5 } from '../../index';
...
function MenuDrawer({ item, menuTextColor }) {
  return (
    <>
      {!item.megaMenu ? (
        <ListItemButton
          {...(item.link && { component: Link, href: item.link, underline: 'none', ...(item?.target && { target: item.target }) })}
          sx={navItemSX}
        >
          <ListItemText primary={item.title} primaryTypographyProps={{ variant: 'caption2', color: menuTextColor || 'text.primary' }} />
        </ListItemButton>
...
function NavList({ item, menuTextColor }) {
  ...
  const currentPath = useLocation(); // Get the current path
